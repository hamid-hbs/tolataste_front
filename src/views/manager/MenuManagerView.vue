...
async function saveItem() {
  ...
  // Récupérer l'id de la catégorie depuis son label
...
async function addCategory() {
  ...
  if (await menu.addCategory(newCategoryName.value.trim())) {
    notif.add({ type: 'success', title: 'Catégorie ajoutée', message: newCategoryName.value })
    newCategoryName.value = ''
  } else {
    notif.add({ type: 'warning', title: 'Impossible d\'ajouter', message: newCategoryName.value })
  }
}
...
<template>
  <div>
    ...
    <div class="mb-6 rounded-2xl bg-white p-5 ring-1 ring-tola-cream-dark/60">
      <h2 class="mb-3 text-sm font-bold uppercase tracking-wider text-tola-gray">Catégories</h2>
      <div class="flex flex-wrap items-center gap-2">
        ...
        <div class="flex items-center gap-1.5">
          <input v-model="newCategoryName" @keyup.enter="addCategory" placeholder="Nouvelle catégorie"
            class="w-32 rounded-lg border border-tola-cream-dark px-2.5 py-1.5 text-xs outline-none focus:border-tola-orange" />
          <button @click="addCategory"
            class="flex items-center gap-1 rounded-lg bg-tola-orange px-2.5 py-1.5 text-xs font-bold text-white"><Plus :size="12" /> Ajouter</button>
        </div>
      </div>
    </div>
